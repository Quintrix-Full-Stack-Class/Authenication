...
public class CryptCurrencyController {
	...
	public List<com.sha.springbootjwtauthorization.crypto.currency.DailyValue> timeSeries(@Argument String currency) throws IOException, InterruptedException, JSONException {
...
//		Object lastRefreshed = json.getJSONObject("Meta Data").get("6. Last Refreshed");
		...
		List<com.sha.springbootjwtauthorization.crypto.currency.DailyValue> dailyValues = new ArrayList<>();
//		for (int i = 0; i < names.length(); i++) {
		for (int i = 0; i < 100; i++) {
			String key = names.getString(i);
			JSONObject valueJSON = timeSeries.getJSONObject(key);
			com.sha.springbootjwtauthorization.crypto.currency.DailyValue dailyValue = new com.sha.springbootjwtauthorization.crypto.currency.DailyValue(key, valueJSON.getDouble("4b. close (USD)"));
	...
	public List<com.sha.springbootjwtauthorization.crypto.currency.News> getNews(@Argument List<String> currencies) throws IOException, InterruptedException, JSONException {
		List<com.sha.springbootjwtauthorization.crypto.currency.News> news = new ArrayList<>();
		...
		for (int i = 0; i < feed.length(); i++) {
			...
			news.add(new com.sha.springbootjwtauthorization.crypto.currency.News(title, url, summary, imageUrl, source, date));
		}
		return news;
	}
	...
	public List<Currency> getCurrencies() {
		return com.sha.springbootjwtauthorization.crypto.currency.Helpers.allCurrencies();
	}
